...
def process_agreement_step(message):
    ...
    msg = bot.send_message(message.chat.id, "Отправьте вашу аватарку:", reply_markup=types.ReplyKeyboardRemove())
...
def change_birthday(message):
    msg = bot.send_message(message.chat.id, "Введите новую дату (ДД.ММ.ГГГГ):", reply_markup=types.ReplyKeyboardRemove())
...
def delete_profile(message):
    ...
    bot.send_message(message.chat.id, "Вы уверены? Это нельзя отменить!", reply_markup=markup)
